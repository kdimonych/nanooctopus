...
- Connection lifecycle via `SocketShutdown`, `SocketAccept`, and `SocketConnector`.
...
- `Socket`: `SocketStream + SocketInfo + SocketShutdown`.
- `ExtendedSocket`: `Socket + SocketReadWith + SocketWriteWith`.
- `SocketAccept`: async accept interface producing implementation-specific sockets.
